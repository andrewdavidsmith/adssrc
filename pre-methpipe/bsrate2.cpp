...
count_states_pos(const bool INCLUDE_CPGS, const string &chrom, 
  ...
  for (size_t i = 0; i < width; ++i, ++position) {
	...
	(!is_guanine(chrom[position+1]) ||
	 ...
	 INCLUDE_CPGS)) {
      ...
      if (is_cytosine(r.seq[i])) {
	++unconv[i];
	++n_cytosines;
      }
      else if (is_thymine(r.seq[i])) {
	++conv[i];
	++n_converted;
	++n_cytosines;
      }
  ...
  read_counts[n_cytosines][n_converted]++;
}
...
main(int argc, const char **argv) {
  ...
  try {
    ...
    vector<vector<size_t> > 
      read_counts(OUTPUT_SIZE, vector<size_t>(OUTPUT_SIZE));
	...
	if (read_counts[i][j] > 0)
	  lim = j + 1;
      for (size_t j = 0; j < lim; ++j) {
	cerr << i << '\t'
	     << j << '\t'
	     << read_counts[i][j] << endl;
      }
      if (lim > 0)
	cerr << endl;
    }
