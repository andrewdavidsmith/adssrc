/* readliftover: lift over mapped reads in bed format using an expanded
 * and non-redundant chain liftover file
 *
...
static bool
lift_pos_strand(const vector<block> &the_blocks, size_t curr_block, site work,
                GenomicRegion &lifted) {

  const size_t n_blocks = the_blocks.size();

  // make a temporary site based on 5' end of read to locate
  const size_t w = lifted.get_width();

  bool lifted_position = false;
  for (size_t i = 0; i < w && !lifted_position; ++i) {
    // find relevant block
...
static bool
lift_neg_strand(const vector<block> &the_blocks, size_t curr_block, site work,
                GenomicRegion &lifted) {
  ...
  if (lifted_position)
    lift_site(the_blocks[last_good_block], last_contained, lifted);
...
main(int argc, const char **argv) {
  try{
    ...
    OptionParser opt_parse(strip_path(argv[0]),
                           "liftover 5'-most site in each interval",
                           "<chain> <intervals>");
    ...
    size_t curr_block = 0;
    GenomicRegion tmp_bam;
    size_t lines_processed = 0;

    while (in_intervals >> tmp_bam) {
      ++lines_processed;

      site work(tmp_bam.get_chrom(), tmp_bam.get_start());
      ...
      GenomicRegion lifted_bam(tmp_bam);
      const bool lift_success = tmp_bam.pos_strand() ?
        lift_pos_strand(the_blocks, local_block, work, lifted_bam) :
        lift_neg_strand(the_blocks, local_block, work, lifted_bam);

      if (lift_success)
        out << lifted_bam << '\n';
      else if (!fails_file.empty())
        fails << tmp_bam << '\n';
    }
    if (VERBOSE)
